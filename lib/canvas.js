...
const _createGLContext = require('gl');
...
function createGLContext(width, height, options) {

  let ctx;

  // First try: without any env changes
  try {
    ctx = _createGLContext(width, height, options);
    if (ctx != null) {
        return ctx;
    }
  } catch (err) {
...
class NodeCanvasElement extends Canvas {
  ...
  getContext(type, options) {
    ...
    if(type === 'webgl' || type === 'webgl2') {
      ...
      console.log('Creating context:', width, height, options);

      const ctx = createGLContext(width, height, options);

      if (!ctx) {
        throw new Error('Failed to create GL context. Are you sure all system dependencies have been installed and/or xvfb is available?');
      }
